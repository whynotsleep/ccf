...
export class WorldMap {
  ...
  initializeSky() {
    ...
    const map = debug.gui.addFolder('地图')
    map.add(effectController, 'turbidity', 0.0, 20.0, 0.1).onChange(guiChanged)
    map.add(effectController, 'rayleigh', 0.0, 4, 0.001).onChange(guiChanged)
    map.add(effectController, 'mieCoefficient', 0.0, 0.1, 0.001).onChange(guiChanged)
    map.add(effectController, 'mieDirectionalG', 0.0, 1, 0.001).onChange(guiChanged)
    map.add(effectController, 'elevation', 0, 90, 0.1).onChange(guiChanged)
    map.add(effectController, 'azimuth', - 180, 180, 0.1).onChange(guiChanged)
    map.add(effectController, 'exposure', 0, 1, 0.0001).onChange(guiChanged)
    guiChanged()
    return this
  }
}
